...
export default ApiService.handleREST({
  async get(req, res: ApiResponse<GroupMemberList>) {
    ...
    const cache = memberListCache[id]
    if (cache !== undefined) {
      const [cacheDate, cacheData] = cache
      if (cacheDate.getTime() >= Date.now() - CACHE_MS) {
    ...
    const data: MidataResponse = await midataResponse.json()
    ...
    const result = data.people
      .map((midataMember) => {
        ...
        }, null as [MidataRoleConfig, number] | null)
        if (role === null) {
    ...
    memberListCache[id] = [new Date(), result]
...
const memberListCache: { [K in GroupId]?: [Date, GroupMemberList] } = {}
