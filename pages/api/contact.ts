...
const fetchALs = async (): Promise<Contact[]> => {
  ...
  return data.people.map((midataPerson) => mapMidataPersonToContact(midataPerson, data))
}
...
const mapMidataPersonToContact = (midataPerson: MidataPerson, data: MidataPeopleResponse): Contact => {
  ...
  return {
    member: {
      id: StringHelper.encode64(midataPerson.id),
