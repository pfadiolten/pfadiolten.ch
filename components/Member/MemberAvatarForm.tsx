...
const MemberAvatarForm: React.FC<Props> = ({
  member,
  role,
  onChange: pushChange,
  onClose: pushClose,
  ...
  const [avatarImage, setAvatarImage] = useState(member.userData.avatar)
  ...
  const handleSubmit = useCallback(async () => {
    if (avatar === null) {
      const error = await FetchService.delete(`users/${member.id}/avatar`)
      if (error !== null) {
        throw error
      }
      pushChange({
    ...
    const [userData, error] = await FetchService.post<UserData>(`users/${member.id}/avatar`, also(new FormData(), (fields) => {
    ...
    pushClose()
  }, [avatar, member, pushChange, pushClose])
  ...
  const handleResetAvatar = useCallback(() => {
    setAvatar(null)
    setAvatarImage(member.userData.avatar)
      ...
      <InputRow>
        ...
        <PreviewBox>
          <MemberCard
            ...
            avatar={avatarImage}
            role={role}
            onResetAvatar={avatarImage === null && member.userData.avatar !== null ? handleResetAvatar : undefined}
...
export default MemberAvatarForm
